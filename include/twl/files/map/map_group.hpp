...
namespace twl
{
	class map_group
	{
		...
		map_group(
			const internal::map_datafile_group* grp,
			const std::vector<mlk::sptr<internal::basic_map_layer>>& layers)
			: m_offset_x{grp->offset_x},
			  ...
			  m_name{grp->version > 2 ? map_constants::ints_to_str(grp->name, 3)
									  : "Group"}
		{
			if(layers.size() >= grp->startlayer + grp->numlayers)
				for(auto i(grp->startlayer);
					i < grp->startlayer + grp->numlayers; ++i)
	...
	public:
		...
		auto offset_x() const noexcept { return m_offset_x; }

		auto offset_y() const noexcept { return m_offset_y; }

		auto parallax_x() const noexcept { return m_parallax_x; }

		auto parallax_y() const noexcept { return m_parallax_y; }

		auto clipping() const noexcept { return m_use_clipping; }

		auto clipping_x() const noexcept { return m_clipping_x; }

		auto clipping_y() const noexcept { return m_clipping_y; }

		auto clipping_w() const noexcept { return m_clipping_w; }

		auto clipping_h() const noexcept { return m_clipping_h; }

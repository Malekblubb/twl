...
namespace twl
{
	class map_image
	{
		...
		map_image(const internal::map_datafile_image* img,
				  const std::vector<mlk::data_packet>& uncompressed_data)
			: m_name{reinterpret_cast<const char*>(
				  uncompressed_data[img->image_name].data())},
			  m_width{static_cast<mlk::st>(img->width)},
			  m_height{static_cast<mlk::st>(img->height)},
			  m_external{static_cast<bool>(img->external)},
			  m_data{img->image_data >= 0 ? uncompressed_data[img->image_data]
										  : mlk::data_packet{}}
		{
		}

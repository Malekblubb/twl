...
namespace twl
{
	class map_layer_tiles : public internal::basic_map_layer
	{
		...
		map_layer_tiles(const internal::map_datafile_layer_tilemap* tlm,
						const std::vector<mlk::data_packet>& uncomoressed_data,
			...
			: internal::basic_map_layer{map_constants::layer_type::tiles},
			  ...
			  m_name{tlm->version > 2 ? map_constants::ints_to_str(tlm->name, 3)
									  : "Tiles"}
		{
			if((tlm->data >= 0) &&
			   uncomoressed_data[tlm->data].size() / sizeof(internal::tile) ==
				   m_width * m_height)
			{
				// add the tiles
				mlk::data_packet tmp(sizeof(internal::tile));
				for(auto i(0); i < uncomoressed_data[tlm->data].size();
					i += sizeof(internal::tile))
				{
					tmp = {std::begin(uncomoressed_data[tlm->data]) + i,
						   std::begin(uncomoressed_data[tlm->data]) + i +
							   sizeof(internal::tile)};
					m_tiles[i / sizeof(internal::tile)] =
						*reinterpret_cast<const internal::tile*>(tmp.data());
				}
			}
		}

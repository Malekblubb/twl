...
namespace twl
{
	namespace internal
	{
		...
		public:
			reply_base(mlk::data_packet&& data) :
		...
		{
			reply() :
				reply_base{{0xff, 0xff, 0xff, 0xff, 'i', 'n', 'f', '3'}}
			{ }
		};
		...
		{
			reply() :
				reply_base{{0xff, 0xff, 0xff, 0xff, 'l', 'i', 's', '2'}}
			{ }
		};
		...
		{
			reply() :
				reply_base{{0xff, 0xff, 0xff, 0xff, 's', 'i', 'z', '2'}}
			{ }
		};
		...
		{return data.size() >= ntw_constants::interaction_datasize() && reply<rep>{}() == mlk::data_packet{std::begin(data) + 6, std::begin(data) + 14};}
	}
}

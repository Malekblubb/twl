...
namespace twl
{
	namespace internal
	{
		...
		{
			auto cpy(data);
			mlk::cnt::insert_front(
				mlk::data_packet{0xff, 0xff, 0xff, 0xff, 0xff, 0xff}, cpy);
			return cpy;
		}

		inline mlk::data_packet add_token(unsigned char token,
		...
		inline std::string from_binary_ip(bool ip4,
										  const mlk::data_packet& data)
		{
			auto buflen(ip4 ? INET_ADDRSTRLEN : INET6_ADDRSTRLEN);
			std::vector<char> buf(buflen);

			inet_ntop(ip4 ? AF_INET : AF_INET6, data.data(), buf.data(),
					  buflen);
			return buf.data();
		}
	}

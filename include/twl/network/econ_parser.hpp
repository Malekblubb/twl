	...
	namespace internal
	{
		...
		private:
			void parse()
			{
				this->get_reply_type();
				if(m_status_reply)
					this->parse_status_reply();
			}

			void get_reply_type()
			{
				m_conn_successful_reply = mlk::stl_string::contains("Authentication successful. External console access granted.", m_workstr);
				m_status_reply = mlk::stl_string::contains("id=", m_workstr) && mlk::stl_string::contains("addr=", m_workstr) &&
								 mlk::stl_string::contains("name=", m_workstr) && mlk::stl_string::contains("id=", m_workstr) &&

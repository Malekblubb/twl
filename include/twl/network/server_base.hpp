...
namespace twl
{
	namespace internal
	{
		...
		public:
			server_base() : on_recved(m_connection.on_recved) {}
			...
			void update()
			{
				...
				for(auto& job : m_jobs) {
					...
					if(m_running_jobs <
					   internal::ntw_constants::max_concurrent_requests())
					{
						if(job.need_resend()) {
							m_connection.send(
								job.addr(), get_request(job.request_type())());
							job.on_resend();
					...
					if(job.need_delete()) --m_running_jobs;
				}

				// remove jobs that are timed out
				mlk::cnt::remove_all_if(
					[](const request_job& job) { return job.need_delete(); },
					m_jobs);
		...
		protected:
			mlk::slot<const mlk::data_packet&, const mlk::ntw::ip_address&>&
				on_recved /*{m_connection.on_recved} GCC pls impl. the standard*/
				;

			float on_job_recv(const mlk::ntw::ip_address& addr)
			{
				auto job(mlk::cnt::find_in_if(
					[&addr](const request_job& job) {
						return job.addr() == addr;
					},
					m_jobs));
				if(job == std::end(m_jobs)) return -1.f;
				job->on_recv();
			...
			void request(const mlk::ntw::ip_address& addr)
			{
				...
				if(!mlk::cnt::exists_if(
					   [&addr](const request_job& job) {
						   return (job.addr() == addr) &&
								  job.request_type() == req;
					   },
					   m_jobs))
					m_jobs.emplace_back(
						addr, timeout,
						internal::ntw_constants::resend_timeouts(), req);
			}

...
namespace twl
{
	namespace internal
	{
		...
		public:
			...
			const server_info& get_result() const noexcept
			{return m_result;}
		...
		private:
			void parse()
			{
				...
				m_workstr = {reinterpret_cast<const char*>(mlk::data_packet{std::begin(m_data) + 17, std::end(m_data)}.data()), m_data.size() - 17};
				...
				for(auto i(0); i < m_result.numclients(); ++i)
					m_result.add_player({this->get_next<std::string>(), this->get_next<std::string>(),
									 this->get_next<int>(), this->get_next<int>(), this->get_next<bool>()});
				m_valid = true;
			}
			...
			T get_next()
			{
				...
				if(nullpos == std::string::npos || nullpos == m_workstr.size() - 1)
					return T{};
